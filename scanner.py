...
def classify(extension: str, config: Config) -> str:
    ...
    if ext in config.FONT_EXTS: return "font"
    if ext in config.DATABASE_EXTS: return "database"
    if ext in config.EXECUTABLE_EXTS: return "executable"
    if ext in config.SYSTEM_EXTS: return "system"
    if ext in config.CONFIG_EXTS: return "config"
    if ext in config.DISK_IMAGE_EXTS: return "disk_image"
    if ext in config.COMPRESSED_ASSET_EXTS: return "game_asset"
